...
if sys.version_info.major == 2:
	...
	import configparser
	import io as cio
>>>>>>> 92f34911dcef6ba9547bfcfe47148190e569d098
...
<<<<<<< HEAD
    ...
    def _env_config(self, section):
        ...
        for n in config:
            config[n] = config[n].replace(
                '$(INIROOT)', os.path.dirname(self.iniload))
        for n in config:
        ...
        if os.path.exists(inipath):
            ...
            for bp in ('include', 'lib'):
                ...
                for n in config['default'][bp].replace(';', ',').split(','):
                    n = os.path.normpath(
                        os.path.join(inihome, self.pathconf(n)))
                    if not self.unix:
                        n = n.replace('\\', '/')
                    data.append("'" + n + "'")
                text = ','.join(data)
    ...
    def loadcfg(self, sect='default', reset=True):
        ...
        def f1(n): return (n[:1] != '\'' or n[-1:] != '\'') and n
        def config(n): return self._getitem(sect, n, '')
    ...
    def parameters(self):
        ...
        wl = ','.join([self.__replace_key(n)
                      for n in self.sequence(self.wlnk)])
        if wl and self.wlnk:
...
<<<<<<< HEAD
    ...
    def _compile_single(self, skipexist, printmode, printcmd):
        ...
        for i in range(len(self._src)):
            ...
            if skipexist and os.path.exists(objname):
                continue
            try:
                os.remove(os.path.abspath(objname))
            except:
                pass
            if printmode & 1:
    ...
    def _compile_threading(self, skipexist, printmode, printcmd, cpus):
        ...
        ctasks = [(os.path.getsize(s), s, o, t)
                  for s, o, t in zip(self._src, self._obj, self._opt)]
        ctasks.sort()
        import threading
        ...
        for n in xrange(cpus):
            ...
            th = threading.Thread(
                target=self._compile_working_thread, args=parameters)
    ...
    def _compile_working_thread(self, skipexist, printmode, printcmd, id):
        ...
        while True:
            ...
            try:
                ...
                pass
            timeslap = time.time()
            output = self.config.compile(
                srcname, objname, options, printcmd, True)
    ...
    def compile(self, skipexist=False, printmode=0, cpus=0):
        ...
        if cpus <= 1:
            ...
            retval = self._compile_threading(
                skipexist, printmode, printcmd, cpus)
        t = time.time() - t
        # print 'time', t
        return retval
...
<<<<<<< HEAD
    ...
    def _process(self, fname, lineno, text):
        ...
        if command in ('cexe', 'clib', 'cdll', 'cwin', 'exe', 'dll', 'win'):
            if not self.int:
                self.int = os.path.abspath(
                    os.path.join('objs', self.config.target))
            self.mode = command[-3:]
            retval = self._process_src(body, fname, lineno)
            return retval
    ...
    def console(self, color):
        ...
        if sys.platform[:3] == 'win':
            ...
            if color >= 0:
                ...
                bold = color & 8
                sys.stdout.write("\033[%s3%d;4%dm" % (
                    bold and "01;" or "", foreground, background))
                sys.stdout.flush()
            else:
...
def main(argv=None):
    ...
    if len(argv) <= 3:
        if name in ('-d', '-cmdline'):
            ...
            print(
                '- envname is a section name in emake.ini which defines environ for this tool')
...
<<<<<<< HEAD
    ...
    def test8():
        ...
        sys.argv = [sys.argv[0], '-r',
                    'd:/acm/aprcode/pixellib/PixelBitmap.cpp']
        main()
    ...
    def test10():
        sys.argv = [sys.argv[0], '-g', 'default',
                    'd:/dev/flash/alchemy5/tutorials/01_HelloWorld/hello.exe', '--version']
        main()
